...
export abstract class TuiBase extends Container implements TUI {
	...
	protected getTerminalFocused(): boolean {
	...
	private setFocusInternal({
		...
		if (isTerminalFocusAware(nextFocus)) {
			nextFocus.terminalFocused = this.terminalFocused;
		}
	...
	start(): void {
		...
		this.terminal.setFocusHandler((focused) => this.handleTerminalFocus(focused));
	...
	private handleTerminalFocus(focused: boolean): void {
